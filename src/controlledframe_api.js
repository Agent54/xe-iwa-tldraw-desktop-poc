...
// The script content that will be injected
const openerPatchScript = `
  console.log('[OpenerPatch] Starting window opener patch initialization');

  if(window.originalOpener===undefined) {


  var actualCode = '(' + function() {
    // To overwrite a global variable

    console.log("inside opener patch script")
    window.foo = 123;
...
class ControlledFrameController {
  constructor() {
    ...
    const patchHeaders = false;
    const addHeaders = false;

    if (patchHeaders) {

      // Add header removal handler to main controlled frame
      ...
      this.controlledFrame.request.onHeadersReceived.addListener(
        function (details) {
          console.log("headers received on main frame", details);
          const filteredHeaders = details.responseHeaders.filter(function (header) {
          ...
          if (addHeaders) {
            ...
            // filteredHeaders.push({
            //   name: 'cross-origin-opener-policy',
            //   value: 'same-origin-plus-corp'
            // });
          }

          console.log("filtered headers for main frame", filteredHeaders);
          return {
  ...
  #onnewwindow(e) {
    ...
    // Create a new controlledframe element
    const newFrame = document.createElement('controlledframe');
    newFrame.id = `popup-${Date.now()}`;
    ...
    // newFrame.setAttribute('sandbox', 'allow-same-origin allow-scripts allow-popups allow-popups-to-escape-sandbox allow-forms');

    // newFrame.setAttribute('coop', 'same-origin-allow-popups');
    //newFrame.setAttribute('coep', 'require-corp');

    const patchHeaders = false;
    const addHeaders = false;
    if (patchHeaders) {

      newFrame.addEventListener('loadstart', function () {
        ...
        newFrame.request.onHeadersReceived.addListener(
          function (details) {
    ...
    // remove csp headers
    //newFrame.contentSecurityPolicy = "script-src 'self'; object-src 'self'";

    const opener = e.window;

    // newFrame.addEventListener('load', (e) => {
    //   opener.attach(newFrame);
    // })

    const applyPatch = false;

    if (applyPatch) {
      // Inject our opener patch
      newFrame.addEventListener('loadcommit', (e) => {
        console.log("new frame loaded !!!!!!!")
        try {
          ...
          // opener.attach(newFrame);
          // injectOpenerPatch();
          // router.registerWindow('windowId', opener);

          console.log("window registered")
          console.log("executing opener patch script", openerPatchScript)
          // Execute our opener patch script
          newFrame.executeScript({
            code: openerPatchScript,
            runAt: 'document_start'
          }).then(() => {
            ``
            console.log("opener patch script executed!")
          }).catch((error) => {
            console.log("failed to execute opener patch script", error)
          })


        } catch (error) {
          // Log.error('Failed to setup window:', );
          console.log("failed to setup window", error)
        }
      });

    }
    // Add to container
    document.getElementById('controlledframe_container').appendChild(newFrame);

    // opener.attach(newFrame);
    console.log("back to how it was")
    ...
    // Attach the window to the new frame

    console.log("attaching window to new frame", e.window, newFrame);
    console.log("this.controlledFrame.contentWindow", this.controlledFrame.contentWindow, this.controlledFrame.contentWindow === e.window);
